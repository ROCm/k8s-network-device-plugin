from "registry.test.pensando.io:5000/k8s-network-device-plugin-build:v1.0"
...
copy "asset-build/device-plugin-asset-push.sh", "/device-plugin-asset-push.sh"
run "chmod +x /device-plugin-asset-push.sh"
...
if user != ""
  ...
  run "echo 'sudo chown -R #{user} /k8s-network-device-plugin/' >> /home/#{user}/.bash_profile"
  run "echo 'sudo chgrp -R #{user} /k8s-network-device-plugin/' >> /home/#{user}/.bash_profile"
...
run "git config --global --add safe.directory ${GOPATH}/src/github.com/ROCm/k8s-network-device-plugin"
